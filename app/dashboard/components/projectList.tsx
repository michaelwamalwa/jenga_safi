...
interface Project {
  ...
  startDate: string;
  endDate: string;
  // Removed the embedded emissions object. This is now tracked separately.
}

interface ProjectListProps {
  projects: Project[];
  onAddProject: (project: Project) => void;
  onProjectSelect?: (projectId: string) => void; // New prop to handle selection
}

export default function ProjectList({ projects, onAddProject, onProjectSelect }: ProjectListProps) {
  ...
  const [newProject, setNewProject] = useState<Partial<Project>>({
    ...
    startDate: "",
    endDate: "",
    // Removed initial emissions state
  });
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold text-emerald-700">Project Portfolio</h3>
        <button
      ...
      {showAddForm && (
        ...
        >
          <h4 className="font-medium text-emerald-800 mb-4 text-lg">Add New Project</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {projects.map((project) => (
          <motion.div
            ...
            onClick={() => onProjectSelect && project._id && onProjectSelect(project._id)} // Make card clickable
          >
            <h4 className="font-bold text-emerald-800 mb-2 text-lg">{project.name}</h4>
            ...
            {/* Removed the emissions summary from the card */}
          </motion.div>
        ))}
      </div>
